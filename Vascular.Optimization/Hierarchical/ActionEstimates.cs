...
public static class ActionEstimates
{
    ...
    /// <param name="x"></param>
    /// <returns></returns>
    public static double MoveBifurcation(HierarchicalCost hc, Branch m, Branch t, Vector3 x)
    {
        var dQt = m.Flow;
        var dQp = -dQt;
        ...
        if (n > 1)
        {
            Span<double> Qs = stackalloc double[n];
            Span<double> Rs = stackalloc double[n];
            Span<double> fs = stackalloc double[n];
            var i = 0;
            foreach (var s in m.Siblings)
            {
                Qs[i] = s.Flow;
                Rs[i] = s.ReducedResistance;
                ++i;
            }
            m.Network.Splitting.Fractions(Rs, Qs, fs);
            var Rp = 0.0;
            for (i = 0; i < n; ++i)
            {
                Rp += Math.Pow(fs[i], 4) / Rs[i];
            }
            Rp = 1.0 / Rp;
            dRp = Rp - p.End.ReducedResistance;
        }
        else
        {
            ...
            var Rp = Lp + RsD;
            dRp = Rp - p.ReducedResistance;
        }

        return
            hc.ReducedResistanceGradient(t) * dRt + hc.FlowGradient(t) * dQt +
            hc.ReducedResistanceGradient(p) * dRp + hc.FlowGradient(p) * dQp;
    ...
    /// <param name="x"></param>
    /// <returns></returns>
    public static double CreateBifurcation(HierarchicalCost hc, Terminal t, Branch f, Vector3 x)
    {
        var dQ = t.Flow;
        ...
        return hc.ReducedResistanceGradient(f) * dR + hc.FlowGradient(f) * dQ;
    ...
    /// <param name="b"></param>
    /// <returns></returns>
    public static double SwapEnds(HierarchicalCost hc, Branch a, Branch b)
    {
        ...
        return
            hc.ReducedResistanceGradient(a) * dRa + hc.FlowGradient(a) * dQa +
            hc.ReducedResistanceGradient(b) * dRb + hc.FlowGradient(b) * dQb;

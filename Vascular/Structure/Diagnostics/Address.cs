...
namespace Vascular.Structure.Diagnostics
{
    ...
    public static class Address
    {
        ...
        /// </exception>
        public static BranchAddress Get(Branch current, Branch? upstream = null)
        {
            ...
            var S = current.Network.Sources[(Source)current.Start];
            return new(S, addr);
        }
        ...
        /// <param name="to"></param>
        /// <returns></returns>
        public static BranchAddress Relative(Branch from, Branch to)
        {
            var gca = Branch.CommonAncestorSafe(from, to);
            // If no common ancestor, use absolute addressing.
            if (gca is null)
            {
                return Get(to);
            }
            ...
            // Now go gca -> end
            addr.AddRange(Get(to, gca).Branch);
            return new(-1, addr);
        }
        ...
        /// </exception>
        public static Branch Navigate(Branch from, BranchAddress address)
        {
            if (address.Source >= 0)
            {
                return Navigate(from.Network, address);
            }

            return Navigate(from, address.Branch);
        }

        public static Branch Navigate(Network net, BranchAddress address)
        {
            var source = net.Sources[address.Source];
            if (source.Root is null)
            {
                throw new TopologyException();
            }
            return Navigate(source.Root, address.Branch);
        }

        private static Branch Navigate(Branch from, IReadOnlyList<int> A)
        {
            for (var i = 0; i < A.Count; i++)
            {
                from = A[i] < 0
                    ? from.GetNthUpstream(-A[i])
                    : from.GetNthChild(A[i]);
            }
            return from;
        }
    }
